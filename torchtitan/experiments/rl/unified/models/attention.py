...
class VLLMAttention(torch.nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        # Unwrap DTensor inputs to local tensors — vLLM's Attention uses
        # local head counts in views/reshapes that are incompatible with
        # DTensor's global shape semantics under torch.compile.
        if self.tp_enabled:
        ...
        # Convert to (batch*seq_len, num_heads, head_dim)
        ...
        # shape under torch.compile.  Narrow back to the original
        # batch_size * seq_len symbols so that downstream DTensor ops
        # (like the wo projection) see shapes consistent with the
        # weight sharding.
