...
Key differences from the base compiler_toolkit:
  ...
  - Optional parallel_dims for TP support (inputs are wrapped as
    DTensor(Replicate) before tracing, matching the base CompiledModule)
"""
...
from torchtitan.experiments.compiler_toolkit.common_utils import parallelize_inputs
...
class RLCompiledModule(torch.nn.Module):
    ...
    RL-specific input padding to a fixed max_seq_len so that the
    AOT-traced graph always sees the same concrete shapes.  Causal
    attention naturally handles right-padding.

    Supports tensor-parallel models: when ``parallel_dims`` is provided,
    plain tensor inputs are wrapped as ``DTensor(Replicate)`` before
    tracing and execution, matching the base CompiledModule behaviour.
    """
    ...
    def _pad_inputs(self, tokens: torch.Tensor) -> tuple[torch.Tensor, int]:
        """Pad tokens to max_seq_len, returning (padded_tokens, orig_len)."""
        ...
        if pad_len > 0:
            return F.pad(tokens, (0, pad_len), value=0), orig_len
        return tokens, orig_len

    def _maybe_parallelize(self, args, kwargs):
        """Wrap inputs as DTensors when running under TP."""
        if self.parallel_dims is not None and self.parallel_dims.tp_enabled:
            return parallelize_inputs(self.parallel_dims, args, kwargs)
        return args, kwargs

    def forward(self, tokens, attention_masks=None):
        padded_tokens, orig_len = self._pad_inputs(tokens)

        args = (padded_tokens,)
        kwargs = {}
        args, kwargs = self._maybe_parallelize(args, kwargs)
        ...
        if self.joint_graph_module is None:
            ...
            if self.cudagraph_warmup_iters > 0:
                ...
                for _ in range(self.cudagraph_warmup_iters):
                    warmup_out = self.joint_graph_module(args, kwargs)
        ...
        output = self.joint_graph_module(args, kwargs)
...
def compile_rl_model(
    ...
    max_seq_len: int,
    parallel_dims: ParallelDims | None = None,
    joint_passes: list[str] | None = None,
    compiler_passes: list[str] | None = None,
    ...
    Args:
        ...
        parallel_dims: Parallel dimensions for TP support. When provided,
            inputs are wrapped as DTensor(Replicate) before tracing. Pass None
            for single-GPU / non-TP use cases.
    ...
    # When TP is active, inputs will be DTensors so validate_dtensor should be True.
    has_tp = parallel_dims is not None and parallel_dims.tp_enabled
